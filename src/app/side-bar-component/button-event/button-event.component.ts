...
export class ButtonEventComponent implements OnInit {
 ...
 @Output() buttonClicked = new EventEmitter();
  ...
  nameValue:string ="Weather Report" ;
  ...
  ngOnInit(){
   ...
   this.filteredOptions = this.countryCtrl.valueChanges 
      .pipe(
        ...
        map(name => name ? this.filtercountry(name) : this.country_list.slice())
      )
   }

    displayFn(option: Country): string {
      return option && option.name ? option.name : '';
     }

    filtercountry(name: string) : Country[] {
        const filterValue = name.toLowerCase();
        return this.country_list.filter(option => option.name.toLowerCase().indexOf(filterValue) === 0);
      } 
  ...
  public str:any;
  public val = this.name;

   showName() {
    console.log('showName clicked.');
    /*this.bodyDiv = true;
    let inputName = document.getElementById('your-input-id');
